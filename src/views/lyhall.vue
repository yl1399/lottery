<template>
	<div class="lyhall">
		<div class="headtip">
			...
			<div class="drawing"><span>{{this.CountdownString}}</span></div>
            <div class="allpeople" @click="showallpeople()">
			    <!-- <i class="iconfont icon-ren-copy-copy"></i> -->
				<span>{{onlineInfo.onlineCount}}</span>
			</div>
		...
		<div class="footertip">
			...
			<ul class="writetxt" v-show="showflag">
				<li>
					<img src="http://113.10.236.174:9999/lyimg/hall/danmu.png" 
						 v-if="!changeflag"
						 id="img1"
				    />
					<!-- <img src="http://113.10.236.174:9999/lyimg/hall/xiazhu.png"
						 @click="changeimg()"
						 v-if="changeflag" 
						 id="img2"
				    /> -->
				...
				<li>
					<input type="text" placeholder="说点什么" v-if="!changeflag" id="barageInput" v-model="barrageContent">
		...
		<div class="outcome_shadow">
            <div class="outcome_show">
				...
				<ul class="outcome_nav">
					<li :class="num == 0 ?'nav_select':''" @click="tabChange(0)">大小单双/合</li>
					<li :class="num == 1 ?'nav_select':''" @click="tabChange(1)">数字玩法</li>
					<li :class="num == 2 ?'nav_select':''" @click="tabChange(2)">数字组合</li>
					<li :class="num == 3 ?'nav_select':''" @click="tabChange(3)">数字定位</li>
				</ul>
				<ul class="outcome_content">
					
					<li class="li_one">
						<ul class="outcome_list1" >
							<li v-for="(item,idx) in ssc_addPayRule" >
						...
						<div class="confirm_btn1">
                            <input type="tel" placeholder="请输入金额" v-model="outcomeMoney1">
					...
					<li class="li_two">
						...
						<ul class="outcome_list2">
							<li v-for="(item,idx) in ssc_numberPayRule">
								<span>{{item.playName}}</span>
							</li>
						</ul>
						<p class="outcome_num2">已选择<span>{{outcomenum2}}</span>注</p>
						<div class="confirm_btn2">
                            <input type="tel" placeholder="请输入金额" v-model="outcomeMoney2" >
					...
					<li class="li_three">
						...
						<div class="confirm_btn3">
                            <input type="tel" placeholder="请输入金额" v-model="outcomeMoney3">
					...
					<li class="li_four">
						<ul class="outcome_list4" >
							<li class="one_box">  
								<div class="f_title">第一球</div>
								<ul class="f_right">
									<li v-for="(item,idx) in ssc_numberPositionRule">
										<span>{{item.playName}}</span>
										<span>{{item.payMultiple}}</span>
									</li>
								</ul>
...
<!-- 
						...
						<div class="confirm_btn4">
                            <input type="tel" placeholder="请输入金额" v-model="outcomeMoney4">
...
<script>
	...
	export default{
		...
		data(){
			return{
				musicOpenSrc:"http://113.10.236.174:9999/lyimg/hall/musicOpen.png",
				musicCloseSrc:"http://113.10.236.174:9999/lyimg/hall/musicClose.png",
				barageOpenSrc:"http://113.10.236.174:9999/lyimg/hall/danmuOpen.png",
				barageCloseSrc:"http://113.10.236.174:9999/lyimg/hall/danmuClose.png",
				changeflag:false,
				barageflag:true,
				showflag:false,
				barrageContent:"", //弹幕内容
				...
				if(this.barageflag == true)
				{
					...
					if(newval){
						...
						function createEle(txt) {
							...
							var colorList=["#f13408","#f1e908","#13f108","#f108ac","#0b0b0b","#efafdc","#08d7f6"];
						    roll.call(oMessage, {
								//call改变函数内部this的指向
		...
		methods:{
			...
			Logout: function() {
				...
				//console.log("Logout:"+this.urlInfo.islogin);
				if(this.userInfo.islogin)
					this.LogoutReq(data);
			},
			GetBarrageMsg()
			...
			ShowOutcomeList1(){
				...
				if(!activeLi.length){
					...
					for(let i =0; i<activeLi.length;i++){
						...
						str += `<li>
									<span>${span1_val}</span>
									<span><input type="tel" value="${span3_val}" class="inputChange" ></span>
					...
					for(let i=0;i<deleteList.length;i++){
						deleteList[i].onclick=function(){
							//console.log(this);
							let moneynum = this.parentNode.children[1].children[0].value*1;
			...
			ShowOutcomeList3(){
				var that = this;
				var activeLi = document.querySelectorAll(".outcome_list3 li ul .click_li3");
				...
				if(!activeLi.length){
					...
					var str = '';
					var span1_val = "";
					this.totalOutcomenum += 1;
				    this.totalOutcomeMoney += 1 * this.outcomeMoney3;
					for(let i =0; i<activeLi.length;i++){
						span1_val += $(activeLi[i]).find("span").eq(0).html();
					}
				    var span3_val = this.outcomeMoney3;
					
					str += `<li>
							<span>组合${span1_val}</span>
							<span><input type="tel" value="${span3_val}" class="inputChange"></span>
							<span class="delete_one"><i class="iconfont icon-cross"></i></span>
						</li>`
			...
			ShowOutcomeList4(){
				...
				if(!activeLi.length){
					...
					for(let i =0; i<activeLi.length;i++){
						var span1_val = $(activeLi[i]).find("span").eq(0).html();
						var span1_val1 = this.positionNum;
						//var span2_val = $(activeLi[i]).find("span").eq(1).html();
						var span3_val = this.outcomeMoney4;
					
						str += `<li>
									<span>定位${span1_val1}/${span1_val}</span>
									<span><input type="tel" value="${span3_val}" class="inputChange"></span>
									<span class="delete_one"><i class="iconfont icon-cross"></i></span>
								</li>`
					}
			...
			commitOutcomeData(){			
				...
				if(confirmList.length>1 && this.confirmFlag == true){
					for(let i=1;i<confirmList.length;i++){
						...
						data.outcomeMoney = $(confirmList[i]).find("span").eq(1).find("input").val();
						msgList.push(data);
					}
					console.log(msgList);
		...
		created(){
            var data={
				 url:this.urlInfo.baseUrl+this.urlInfo.lotteryoddsPath
			}
			if(this.ssc_addPayRule.length==0)
				this.GetLotteryOdds(data);
		},
	    mounted:function(){
			this.GetShowPageInfo(1);
			$(".outcome_list1 li:last-child").width("100%");
			var that = this;
	    	//默认开启背景音乐
	    	this.processMusic();
		   ...
		   this.getHallCountTimer = setInterval(() => {
				this.UpdateCountdown({min:this.CountdownInfo.min, sec:this.CountdownInfo.sec});
			}, 1000)

	    	//console.log(this.urlInfo.islogin);
	    	if(this.userInfo.islogin){
               this.getHallTimer = setInterval(()=>{
	    		...
	    		this.SetHallTimer(this.getHallTimer);
	    	}
	    	//**显示弹幕区，下注区，系统消息区**//
			...
			setTimeout(function(){
				...
				//极大极小玩法
				var  li_list3 = document.querySelectorAll(".one1_box li");
				for(let i =0; i<li_list3.length; i++){
					let li_num3 = 0;
					li_list3[i].onclick = function(){
						$(this).addClass("click_li3").siblings().removeClass("click_li3");
						that.outcomenum3 = 1;
					}
				}

					var  li_list3 = document.querySelectorAll(".two1_box li");
				for(let i =0; i<li_list3.length; i++){
					let li_num3 = 0;
					li_list3[i].onclick = function(){
						$(this).addClass("click_li3").siblings().removeClass("click_li3");
					...
					var  li_list3 = document.querySelectorAll(".three1_box li");
				for(let i =0; i<li_list3.length; i++){
					let li_num3 = 0;
					li_list3[i].onclick = function(){
						$(this).addClass("click_li3").siblings().removeClass("click_li3");
				...
				var  li_list3 = document.querySelectorAll(".four1_box li");
				for(let i =0; i<li_list3.length; i++){
					let li_num3 = 0;
					li_list3[i].onclick = function(){
						$(this).addClass("click_li3").siblings().removeClass("click_li3");
				...
				var  li_list3 = document.querySelectorAll(".five1_box li");
				for(let i =0; i<li_list3.length; i++){
					let li_num3 = 0;
					li_list3[i].onclick = function(){
						$(this).addClass("click_li3").siblings().removeClass("click_li3");
				...
				//数字定位玩法
				var  li_list4 = document.querySelectorAll(".outcome_list4 li ul li");
				//console.log(li_list4);
				for(let i =0; i<li_list4.length; i++){	    
					li_list4[i].onclick = function(){
						$(this).addClass("click_li4").siblings().removeClass("click_li4");
						$(this).parent().parent().siblings().find("ul").find("li").removeClass("click_li4");
						var ss = $(this).parent().parent().attr("class").split("_")[0];   //确定是那个球
						if(ss == "one"){
							that.positionNum = 1;
						}else if(ss == "two"){
							that.positionNum = 2;
